...
describe("EffectNode", () => {
  ...
  // End-to-end test: when a tracked pulse changes, the efect should
  // re-run on the next microtask with the new value. This excercises
  // the full pipeline: pulse.set() → mark() → schedule() → flush() → run().
  it("re-runs when pulse dependency changes", async () => {
    ...
    // The scheduler uses queueMicrotask, so we awiat one tick
